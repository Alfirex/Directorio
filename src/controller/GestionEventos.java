...
public class GestionEventos {
	...
	actionListener_recuperarAnyo,actionListener_comparaPalabra;
	...
	//Examen
		...
		actionListener_comparaPalabra = new ActionListener() {
			
			public void actionPerformed(ActionEvent actionEvent) {	
				System.out.println("Has pulsado Compara caracteres");
			
				try {
					String res = call_compararContenidoPalabra();
				} catch (FileNotFoundException e) {
					// TODO Auto-generated catch block
					view.showError("No ha encontrado nada ");
				}
				
			}
		};
		view.getBtnCompararPalabra().addActionListener(actionListener_comparaPalabra);
		
		actionListener_recuperarAnyo = new ActionListener() {
			public void actionPerformed(ActionEvent actionEvent) {
				try {
					call_recuperarAnyo();
				} catch (ClassNotFoundException ex) {
		...
		view.getBtnAnyo().addActionListener(actionListener_recuperarAnyo);
	}
	...
	private void call_recuperarAnyo() throws FileNotFoundException, ClassNotFoundException, IOException {
		String identificador = view.getTxtNumeroDePaginas().getText();
	...
	private String call_compararContenidoPalabra() throws FileNotFoundException {
		String fichero1 = view.getFichero1().getText(); //almacena el nombre de fichero1
		int palabra = Integer.parseInt(view.getFichero2().getText()); //almacena el nombre de fichero2
		
		try {
			//Mostramos el nombre del fichero1 y fichero 2 dentro del .compararContenido()
			String res = model.compararPalabras(fichero1, palabra );
			
			view.getTextArea().setText(res);
